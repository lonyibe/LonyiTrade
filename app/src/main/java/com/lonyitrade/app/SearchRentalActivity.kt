                ...
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        ...
                        if (propertyType != null) {
                            rentals = rentals.filter { it.propertyType.equals(propertyType, ignoreCase = true) }
                        }
                        if (!minRent.isNullOrEmpty()) {
                            rentals = rentals.filter { it.monthlyRent != null && it.monthlyRent >= minRent.toDouble() }
                        }
                        if (!maxRent.isNullOrEmpty()) {
                            rentals = rentals.filter { it.monthlyRent != null && it.monthlyRent <= maxRent.toDouble() }
                        }
